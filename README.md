...
> **一句话**：独立设计并实现的**生产级 AI 知识库平台**，打通 **RAG（真实语义嵌入 + 混合检索 + 两阶段重排序）、ReAct 智能体、多模型 LLM、上下文记忆、知识图谱（含图增强检索）** 五大能力；支持在 **Web 端接入 / 切换大模型密钥并脱密保护**，前后端分离 + 分层架构，Docker 一键部署。
...
| 模块 | 能力 |
| --- | --- |
| **RAG 检索增强** | 多格式文档（PDF/Word/TXT/Markdown）上传 → 解析 → 递归分块 → 向量化入库；**混合检索**（向量稠密 + BM25 稀疏 + RRF 融合）+ **两阶段重排**（向量召回 + Cross-Encoder 精排）；相关性阈值过滤，无据不编造 |
